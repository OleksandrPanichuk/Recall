...
тягнеться місяцями. Коли з'явиться spaced repetition, він планує по питанню —
і кожен напрямок отримає свою криву забування.
...
Кожне питання зберігає посилання на свій item, тому виправлення перекладу
згодом полагодить обидва напрямки, а не один.

## Повторення

Набір, який ти пройшов, повертається за розкладом: **через день → 3 дні →
тиждень → 2 тижні → місяць**, далі кожен місяць. Пункт меню `🔁 Повторення`
...
- **стеля** (`maxIntervalDays`) — далі інтервал не росте. Постав 7, і набір
  повертатиметься щотижня назавжди.
- **ліміт** (`maxRepetitions`) — після стількох проходжень набір «на пенсії»
  і більше не пропонується.
...
Налаштування резолвляться: під набір → глобальні → вбудовані.
