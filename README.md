...
Що можна робити з браузера:

- **Набори** — створити, перейменувати, змінити опис/джерело, перекласти в іншу
  папку, опублікувати або відправити в архів.
- **Питання** — додати, відредагувати (тип незмінний, бо історія відповідей
  прив'язана до питання), видалити. Питання, на яке вже відповідали, видалити
  не можна — API відповідає помилкою, і вона видна в інтерфейсі.
- **Лексика** — додати пару, виправити терміни чи переклади; питання, згенеровані
  з пари, перебудовуються автоматично.
- **Папки** — створити, перейменувати, перемістити, видалити (порожню).
- **Налаштування** — глобальні та per-set: перемішування варіантів і питань,
  режим екзамену, інтервали повторень. Per-set override видно як `set`, і його
  можна повернути до `global` однією кнопкою.
- **Статистика** — точність набору й розбивка за темами.
