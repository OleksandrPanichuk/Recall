...
import { mailerFor } from "./notifications.module";
import type { Letter } from "./notifications.port";
...
describe("mailerFor", () => {
	test("logs the whole letter when no smtp is configured, so the link is reachable locally", async () => {
		...
		await mailerFor(environment(), logger).send(letter);
	...
	test("says so rather than pretending to have sent", () => {
		...
		expect(mailerFor(environment(), logger).constructor.name).toBe("LogMailer");
	});

	test("sends over smtp once a url is configured", () => {
		const { logger, warnings } = recordingLogger();
		const mailer = mailerFor(environment("smtp://127.0.0.1:55025"), logger);
